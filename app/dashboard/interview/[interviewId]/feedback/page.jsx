...
const FeedBack = ({params}) => {
  ...
  return (
    <div className='p-10'>
      ...
      <>
      <h2 className='text-primary text-lg my-3'>Your overall interview rating: <string>7/10</string></h2> 
      ...
      {feedbackList&&feedbackList.map((item, index)=>(
        ...
        {item.question} <ChevronsUpDown className='h-7 w-7'/>
        </CollapsibleTrigger>      
        <CollapsibleContent>
          <div className='flex flex-col gap-2'>
            ...
            <h2 className='bg-blue-50 p-2 border rounded-lg text-sm text-blue-800'><strong>Feedback: </strong>{item.feedback}</h2>
          </div>
        </CollapsibleContent>
      </Collapsible>
     ...
     <Button onClick={()=>router.replace('/dashboard')} className="bg-blue-950 mt-8">Go Home</Button>
    </div>
  )
}
